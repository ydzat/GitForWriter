...
- **enableCache**: 启用后会缓存 AI 分析结果，相同内容不会重复调用 API（仅对 `unified` provider 生效）
- **cacheTTL**: 缓存有效期，过期后会重新调用 API 获取最新结果（仅对 `unified` provider 生效）
- **cacheMaxSize**: 缓存占用的最大内存，超出后会自动清理最久未使用的条目（仅对 `unified` provider 生效）

> **注意**：缓存配置仅对 `unified` provider 生效。其他 provider（openai、claude）暂不支持缓存配置。
